...
def extract_tract(
    ...
    **kwargs,
    ...
    excl_rois = [
        mrtrix.tckedit_exclude_params(spec=mrtrix.tckedit_various_file_params_(fpath))
        for fpath in input_data["anat"]["rois"]["exclusion"]
    ]
    truncate_rois = [
        mrtrix.tckedit_mask_params(spec=mrtrix.tckedit_various_file_params_2(fpath))
